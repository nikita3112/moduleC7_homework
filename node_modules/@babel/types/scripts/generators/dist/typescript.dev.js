...
var toFunctionName = require("../utils/toFunctionName"); // For backward compat, we cannot use TS 3.7 syntax in published packages


var ts3_7 = process.argv.includes("--ts3.7"); // TypeScript 3.7: https://github.com/microsoft/TypeScript/pull/32695 will allow assert declarations

var asserts = ts3_7 ? function (assertion) {
  return "asserts ".concat(assertion);
} : function () {
  return "void";
};
...
try {
  for (var _iterator = t.TYPES[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
    ...
    lines.push("export function assert".concat(typeName, "(node: object | null | undefined, opts?: object | null): ").concat(asserts(result === "boolean" ? "node" : result), ";"));
  }
} catch (err) {
...
lines.push( // assert/
"export function assertNode(obj: any): ".concat(asserts("obj is Node")), // builders/
...
"export function validate<T extends Node, K extends keyof T>(n: Node | null | undefined, key: K, value: T[K]): ".concat(asserts("n is T")), "export function validate(n: Node, key: string, value: any): void;");
