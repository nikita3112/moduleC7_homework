...
var flow = function flow(superClass) {
  ...
  function (_superClass2) {
    ...
    function _temp(options, input) {
      ...
      _this2 = _possibleConstructorReturn(this, _getPrototypeOf(_temp).call(this, options, input));
...
function (_Scope) {
  ...
  function TypeScriptScope() {
    ...
    _this15 = _possibleConstructorReturn(this, (_getPrototypeOf2 = _getPrototypeOf(TypeScriptScope)).call.apply(_getPrototypeOf2, [this].concat(args)));
    _this15.types = [];
    _this15.enums = [];
    _this15.constEnums = [];
    _this15.classes = [];
...
function (_ParserError) {
  ...
  function Tokenizer(options, input) {
    ...
    _this37 = _possibleConstructorReturn(this, _getPrototypeOf(Tokenizer).call(this));
...
function (_ExpressionScope) {
  ...
  function ArrowHeadParsingScope(type) {
    ...
    _this39 = _possibleConstructorReturn(this, _getPrototypeOf(ArrowHeadParsingScope).call(this, type));
    _this39.errors = new Map();
    return _this39;
  }
...
function (_LValParser) {
  ...
  _createClass(ExpressionParser, [{
    ...
    value: function parseExpression(disallowIn, refExpressionErrors) {
      ...
      if (disallowIn) {
        return this.disallowInAnd(function () {
          return _this41.parseExpressionBase(refExpressionErrors);
        });
      }
    ...
    value: function parseMaybeAssignDisallowIn(refExpressionErrors, afterLeftParse, refNeedsArrowPos) {
      ...
      return this.disallowInAnd(function () {
        return _this42.parseMaybeAssign(refExpressionErrors, afterLeftParse, refNeedsArrowPos);
      });
    }
  }, {
          ...
          switch (this.getPluginOption("pipelineOperator", "proposal")) {
            case "smart":
              return this.withTopicPermittingContext(function () {
                return _this44.parseSmartPipelineBody(_this44.parseExprOpBaseRightExpr(op, prec), startPos, startLoc);
              });

            case "fsharp":
              return this.withSoloAwaitPermittingContext(function () {
                return _this44.parseFSharpPipelineBody(prec);
              });
          }
    ...
    value: function parseFunctionBody(node, allowExpression) {
      ...
      if (isExpression) {
        ...
        node.body = this.parseBlock(true, false, function (hasStrictModeDirective) {
          var nonSimple = !_this45.isSimpleParamList(node.params);

          if (hasStrictModeDirective && nonSimple) {
            var errorPos = (node.kind === "method" || node.kind === "constructor") && !!node.key ? node.key.end : node.start;

            _this45.raise(errorPos, ErrorMessages.IllegalLanguageModeDirective);
          }

          var strictModeChanged = !oldStrict && _this45.state.strict;

          _this45.checkParams(node, !_this45.state.strict && !allowExpression && !isMethod && !nonSimple, allowExpression, strictModeChanged);

          if (_this45.state.strict && node.id) {
            _this45.checkLVal(node.id, "function name", BIND_OUTSIDE, undefined, undefined, strictModeChanged);
          }
        });
        this.prodParam.exit();
        this.expressionScope.exit();
  ...
  _createClass(StatementParser, [{
    ...
    value: function parseDoStatement(node) {
      ...
      node.body = this.withTopicForbiddingContext(function () {
        return _this46.parseStatement("do");
      });
    ...
    value: function parseTryStatement(node) {
      ...
      if (this.match(types._catch)) {
        ...
        clause.body = this.withTopicForbiddingContext(function () {
          return _this47.parseBlock(false, false);
        });
        this.scope.exit();
    ...
    value: function parseWhileStatement(node) {
      ...
      this.state.labels.push(loopLabel);
      node.body = this.withTopicForbiddingContext(function () {
        return _this48.parseStatement("while");
      });
      this.state.labels.pop();
    ...
    value: function parseWithStatement(node) {
      ...
      node.body = this.withTopicForbiddingContext(function () {
        return _this49.parseStatement("with");
      });
    ...
    value: function parseFor(node, init) {
      ...
      this.expect(types.parenR);
      node.body = this.withTopicForbiddingContext(function () {
        return _this50.parseStatement("for");
      });
      this.scope.exit();
      this.state.labels.pop();
    ...
    value: function parseFunction(node) {
      ...
      this.withTopicForbiddingContext(function () {
        _this52.parseFunctionBodyAndFinish(node, isStatement ? "FunctionDeclaration" : "FunctionExpression");
      });
      this.prodParam.exit();
      this.scope.exit();
    ...
    value: function parseClassBody(constructorAllowsSuper, oldStrict) {
      ...
      this.expect(types.braceL);
      this.withTopicForbiddingContext(function () {
        while (!_this53.match(types.braceR)) {
          if (_this53.eat(types.semi)) {
            if (decorators.length > 0) {
              throw _this53.raise(_this53.state.lastTokEnd, ErrorMessages.DecoratorSemicolon);
            }

            continue;
          }

          if (_this53.match(types.at)) {
            decorators.push(_this53.parseDecorator());
            continue;
          }

          var member = _this53.startNode();

          if (decorators.length) {
            member.decorators = decorators;

            _this53.resetStartLocationFromNode(member, decorators[0]);

            decorators = [];
          }

          _this53.parseClassMember(classBody, member, state);

          if (member.kind === "constructor" && member.decorators && member.decorators.length > 0) {
            _this53.raise(member.start, ErrorMessages.DecoratorConstructor);
          }
        }
      });
...
function (_StatementParser) {
  ...
  function Parser(options, input) {
    ...
    _this54 = _possibleConstructorReturn(this, _getPrototypeOf(Parser).call(this, options, input));

    var ScopeHandler = _this54.getScopeHandler();

    _this54.options = options;
    _this54.inModule = _this54.options.sourceType === "module";
    _this54.scope = new ScopeHandler(_this54.raise.bind(_assertThisInitialized(_this54)), _this54.inModule);
    _this54.prodParam = new ProductionParameterHandler();
    _this54.classScope = new ClassScopeHandler(_this54.raise.bind(_assertThisInitialized(_this54)));
    _this54.expressionScope = new ExpressionScopeHandler(_this54.raise.bind(_assertThisInitialized(_this54)));
    _this54.plugins = pluginsMap(_this54.options.plugins);
    _this54.filename = options.sourceFilename;
    return _this54;
  }
