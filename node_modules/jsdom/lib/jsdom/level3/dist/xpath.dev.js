...
module.exports = function (core) {
  ...
  function mergeNodeLists(x, y) {
    var a,
        ...
        banc,
        r = [];
    if ('object' !== _typeof(x)) throw new XPathException(XPathException.INVALID_EXPRESSION_ERR, 'Invalid LHS for | operator ' + '(expected node-set): ' + x);
    if ('object' !== _typeof(y)) throw new XPathException(XPathException.INVALID_EXPRESSION_ERR, 'Invalid LHS for | operator ' + '(expected node-set): ' + y);
  ...
  function comparisonHelper(test, x, y, isNumericComparison) {
    ...
    if ('object' === _typeof(x) && 'object' === _typeof(y)) {
      var aMap = {};
      ...
      return false;
    } else if ('object' === _typeof(x) && x.nodes && x.nodes.length) {
      for (var i = 0; i < x.nodes.length; ++i) {
        var xi = coersion({
          nodes: [x.nodes[i]]
        }),
      ...
      return false;
    } else if ('object' === _typeof(y) && x.nodes && x.nodes.length) {
      for (var i = 0; i < x.nodes.length; ++i) {
        var yi = coersion({
          nodes: [y.nodes[i]]
        }),
  ...
  var fn = {
    ...
    'boolean': function booleanVal(x) {
      return 'object' === _typeof(x) ? x.nodes.length > 0 : !!x;
    },
    'last': function last() {
    ...
    'count': function count(nodeSet) {
      if ('object' !== _typeof(nodeSet)) throw new XPathException(XPathException.INVALID_EXPRESSION_ERR, 'Position ' + stream.position() + ': Function count(node-set) ' + 'got wrong argument type: ' + nodeSet);
  ...
  var stringifyObject = xpath.stringifyObject = function stringifyObject(ctx) {
    ...
    function helper(ctx) {
      ...
      if ('object' !== _typeof(ctx)) return ctx;
