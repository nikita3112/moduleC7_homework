"use strict"; // This files implements the calculation of the offset between the global monotonic clock and UNIX time. This value is
...
function calculateClockOffset() {
  var start = Date.now();
  var cur = start; // Limit the iterations, just in case we're running in an environment where Date.now() has been mocked and is
  // constant.

  for (var i = 0; i < 1e6 && cur === start; i++) {
    cur = Date.now();
